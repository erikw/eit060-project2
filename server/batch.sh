...
#generate key
keytool -keystore $KEYSTORE_FILE -storepass $PASSWD -genkeypair -alias $NAME -keysize 1024 -keypass $PASSWD
# generate csr
keytool -keystore $KEYSTORE_FILE -storepass $PASSWD -certreq -alias $NAME -file $CSR_PATH -keypass $PASSWD
# sign with CA
openssl x509 -req -in $CSR_PATH -CA $CA_CERT -CAkey $CA_KEY -out $CRT_PATH -days 365  -CAcreateserial -CAserial $CA_SEQ
# import CA cert to keystore
keytool -keystore $KEYSTORE_FILE -storepass $PASSWD -importcert -alias root -file $CA_CERT
# import signed cert
keytool -keystore $KEYSTORE_FILE -storepass $PASSWD -importcert -alias $NAME -keypass $PASSWD -file $CRT_PATH

# import CA cert to truststore
keytool -keystore $TRUSTSTORE_FILE -storepass $TRUST_PASSWD -importcert -alias root -file $CA_CERT
